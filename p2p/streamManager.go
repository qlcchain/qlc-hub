...
import (
	...
	version2 "github.com/qlcchain/qlc-hub"
...
func (sm *StreamManager) SetQlcNodeAndMaxStreamNum(node *Node) {
	sm.node = node
...
func (sm *StreamManager) BroadcastMessage(messageName MessageType, v interface{}) {
	...
	version := p2pVersion
	message := NewHubMessage(messageContent, byte(version), messageName)
