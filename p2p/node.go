...
import (
	...
	"github.com/qlcchain/qlc-hub/p2p/pubsub"
)
...
func (node *Node) buildHost() error {
	var err error
	go node.getBootNode(node.cfg.P2P.BootNodes)
	node.logger.Info("Start hub Host...")
	...
	if err := node.host.Peerstore().AddPrivKey(node.ID, node.privateKey); err != nil {
		return err
	}
	if err := node.host.Peerstore().AddPubKey(node.ID, node.privateKey.GetPublic()); err != nil {
		return err
	}
...
func (node *Node) startLocalDiscovery() error {
...
func (node *Node) startPingService() {
...
func (node *Node) publishPeersInfoToChainContext() {
...
func (node *Node) connectBootstrap(pInfoS []peer.AddrInfo) {
	...
	_, err = node.dis.Advertise(node.ctx, ProtocolFOUND)
	if err != nil {
		node.logger.Error(err)
		return
	}
...
func (node *Node) startPeerDiscovery() {
...
// findPeers
func (node *Node) findPeers() error {
...
func (node *Node) updatePeersInfo(peers []peer.AddrInfo) {
	node.streamManager.onlinePeersInfo.Range(func(key, value interface{}) bool {
		k := key.(string)
		var i int
...
func (node *Node) SetP2pService(ns *P2pService) {
	node.netService = ns
}
...
func (node *Node) BroadcastMessage(messageName MessageType, value interface{}) {
	node.streamManager.BroadcastMessage(messageName, value)
}

// SendMessageToPeer send message to a peer.
func (node *Node) SendMessageToPeer(messageName MessageType, value interface{}, peerID string) error {
...
func (node *Node) processMessage(ctx context.Context, pubSubMsg pubsub.Message) error {
	...
	message, err = ParseHubMessage(data)
	if err != nil {
		return err
	}
	node.logger.Debug("message Type is :", message.messageType)
	messageBuffer := data[MessageHeaderLength:]
...
func (node *Node) getBootNode(urls []string) {
